...
func (r RepoUsuarios) BuscarTodasPropriedades() ([]models.Propriedade, error) {
	rows, err := r.db.Query("SELECT propriedades.id, propriedades.nome, propriedades.descricao, propriedades.estado, propriedades.cidade, propriedades.categoria, propriedades.dono, usuarios.nome, usuarios.nome_do_meio, usuarios.ultimo_nome FROM propriedades left join usuarios on usuarios.id = propriedades.dono;")
	if err != nil {
